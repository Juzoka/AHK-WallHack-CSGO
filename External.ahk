...
hwnd := _ImGui_GUICreate("External by @nottnk", 720, 540, (A_ScreenWidth-720)//2, (A_ScreenHeight-540)//2)
winshow, ahk_id %hwnd%
Global rc
...
Loop {
	...
	if !(enable_dont_render & !WinActive("External by @nottnk"))
		settings_gui()
	IsInGame := IsInGame()
...
settings_gui() {
	...
	*/
	...
	if _ImGui_BeginTabItem("debug") {
		...
		_ImGui_Checkbox("Trash AimBot", enable_trash_aimbot)

		_ImGui_Text("Need Help? : @nottnk")
